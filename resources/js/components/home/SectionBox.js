...
const SectionBox = ({ tint, image, title, desc }) => {
    return (
        <Grid container my={'2rem'} p={5} spacing={2} justifyContent="center" alignItems="center" borderRadius={'4px'} bgcolor={tint ? '#f1f1f1' : 'white'}>
            ...
            <Grid item xs={12} md={6}>
                ...
                <Button variant={title === 'Property Listings' ? 'outlined' : 'contained'} endIcon={<ArrowCircleRightIcon />} sx={{ mt: '1rem' }}>Take me there</Button>
            </Grid>
            {tint && <Grid item xs={12} md={6} textAlign='center'>
                <Box component={'img'} width={'100%'} src={image} />
            </Grid>}
